...
// ─── Motor ÚNICO de score financiero ─────────────────────────────────────────
// Antes existían dos motores independientes (este de 5 factores y otro de
// penalizaciones en financialService) que podían dar resultados distintos para
// el mismo usuario en pantallas distintas — eso destruye confianza. Este es el
// único motor: /score y el dashboard consumen el mismo cálculo.
const calculateScore = async (userId) => {
  ...
  return { score: totalScore, label, color, components, recommendations };
};

// Vista compacta para el dashboard: mismo score, expresado como grado A-F con
// etiqueta humana y los tips de los componentes flojos como insights.
// Umbrales alineados con la clasificación de arriba (80/60/40) para que el
// dashboard y la página de score NUNCA se contradigan.
const getDashboardScore = async (userId) => {
  const { score, components } = await calculateScore(userId);

  let grade, gradeLabel;
  if      (score >= 80) { grade = 'A'; gradeLabel = 'Excelente'; }
  else if (score >= 60) { grade = 'B'; gradeLabel = 'Bueno'; }
  else if (score >= 40) { grade = 'C'; gradeLabel = 'Regular'; }
  else if (score >= 25) { grade = 'D'; gradeLabel = 'Necesita atención'; }
  else                  { grade = 'F'; gradeLabel = 'Crítico'; }

  const insights = components
    .filter((c) => c.score / c.max < 2 / 3)
    .map((c) => ({
      type: c.score / c.max < 1 / 3 ? 'critical' : 'warning',
      message: c.tip,
    }));

  return { score, grade, gradeLabel, insights };
};

module.exports = { calculateScore, getDashboardScore };
